...
// ดีดนิ้วชี้ (โหมด 'flick' — เฉพาะเกมไพ่จับคู่ ข้อ 2 ของรอบนี้): งอนิ้วชี้ค้างไว้
...
// [ค่า threshold เป็นค่าประมาณ ยังไม่ผ่านทดสอบมือจริง ต้องปรับจูนบนอุปกรณ์จริงก่อน]
const FLICK_CURL_ON = 0.55;       // extension (ดูฟังก์ชัน indexExtension) ต่ำกว่านี้ = นิ้วงอ พร้อมดีด
const FLICK_EXTEND_ON = 0.85;     // extension สูงกว่านี้ = นิ้วยืดออกแล้ว (fire)
const FLICK_MIN_FRAMES_ARMED = 2; // ต้องงอต่อเนื่องกี่เฟรมก่อนนับว่า armed จริง (กัน false arm เฟรมเดียว)
const FLICK_MAX_ARM_AGE_MS = 900; // armed ไว้นานเกินนี้โดยไม่ยืดออก = ยกเลิก armed (กันค้างถาวร)
...
export async function createHandPinchInput(fxCanvas, handlers, onCameraLost) {
  ...
  function updateFlickState(ext, x, y, tx, ty) {
    ...
    if (ext < FLICK_CURL_ON) {
    ...
    } else if (ext > FLICK_EXTEND_ON) {
      // นิ้วยืดออกแล้ว — ยิงเฉพาะตอน armed มาก่อนจริงๆ ภายในเวลาที่กำหนด และเว้น
      // คูลดาวน์กันดีดรัวถี่เกินไปจากเฟรมต่อเนื่องที่ยังยืดค้างอยู่
      if (flickArmed && (now - flickArmedAt) < FLICK_MAX_ARM_AGE_MS && (now - lastFlickAt) > FLICK_COOLDOWN_MS) {
        lastFlickAt = now;
        handlers.onPick && handlers.onPick(sx, sy, GRAB_SLOP, { x: stx, y: sty });
      }
      flickArmed = false;
      ...
      // ช่วงกลาง (ไม่งอพอจะ arm, ไม่ยืดพอจะ fire) — armed ค้างนานเกินไปโดยไม่ดีด
      // จริง (เช่นเด็กงอนิ้วค้างไว้เฉยๆ) ให้ยกเลิก กันค้างสถานะ armed ตลอดกาล
      flickArmedFrames = 0;
      if (flickArmed && (now - flickArmedAt) > FLICK_MAX_ARM_AGE_MS) flickArmed = false;
    }
    lastX = sx; lastY = sy;
    lastTx = stx; lastTy = sty;
  }
  ...
  function detectLoop(gen) {
    ...
    if (!paused && !videoEl.paused && videoEl.currentTime !== lastVideoTime) {
      ...
      if (hand) {
        ...
        if (mode === 'flick') {
          // โหมดเกมไพ่ (ข้อ 2 ของรอบนี้): ดีดนิ้วชี้แทนจีบสองนิ้ว
