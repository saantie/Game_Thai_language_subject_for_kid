...
// ดีดนิ้วชี้ (โหมด 'flick' — เฉพาะเกมไพ่จับคู่): ปลายนิ้วชี้อยู่ใกล้นิ้วโป้งก่อน
// (armed, ท่าเตรียมดีดคล้ายจีบหลวมๆ) แล้วปลายนิ้วชี้ห่างออกจากนิ้วโป้งชัดเจน
// (fire) ต่างจากท่าจีบสองนิ้วที่ใช้ในเกมหยิบฟอง (โหมด 'pinch' เดิม) ตรงที่จีบ
// จับค้างไว้แล้วปล่อยตอนวาง แต่ดีดคือ "แยกออกจากกันแล้วจบทันที" ไม่มีจับค้าง
//
// ประวัติการปรับจูน (ทั้งหมดมาจากทดสอบเครื่องจริง):
//   รอบ 1-3: วัดจาก "นิ้วชี้งอ/ยืดเทียบกับโคนนิ้วชี้ตัวเอง" (ไม่เกี่ยวกับนิ้วโป้ง
//     เลย) รอบ 1 ตึงเกินไป (ไม่ตอบสนอง) รอบ 2 หลวมเกินไป+มีทางลัดความเร็ว (ไพ่
//     ลอยไปเองจากแค่ขยับมือเร็ว) รอบ 3 ตัดทางลัดออกแล้วปรับ threshold กลางๆ
//   รอบ 4 (ปัจจุบัน): พบว่ามือที่กำอยู่เฉยๆ (ไม่ได้ยืดนิ้วชี้จริง) แค่หมุนข้อมือ
//     เล็กน้อยก็ยังทำให้ไพ่ถูกเลือก — เพราะ "นิ้วชี้เทียบกับโคนนิ้วชี้ตัวเอง" เป็น
//     ค่าที่ไวต่อมุมกล้อง/การหมุนข้อมือมากเกินไป (landmark เพี้ยนได้ง่ายตอนหมุน)
//     เปลี่ยนมาวัด "ระยะปลายนิ้วชี้ห่างจากปลายนิ้วโป้ง" แทน (สูตรเดียวกับ
//     getPinchState แต่ตีความกลับด้าน: ใกล้=armed, ไกล=fire) เพราะระยะระหว่าง
//     2 จุดบนมือเดียวกันไม่เปลี่ยนตามการหมุนข้อมือทั้งมือ (มือกำนิ่งๆ หมุนไปมา
//     ระยะนิ้วชี้-นิ้วโป้งไม่ควรเปลี่ยน) ต้องมีการ "แยกสองนิ้วออกจากกันจริง" เท่านั้น
//     ถึงจะยิง ตรงกับความหมายจริงของ "ดีดนิ้ว" ที่ต้องมีนิ้วโป้งเป็นจุดอ้างอิงด้วย
// [ค่าทั้งหมดยังเป็นค่าประมาณ ต้องทดสอบบนอุปกรณ์จริงต่อ]
const FLICK_NEAR_ON = 0.40;        // ระยะนิ้วชี้-นิ้วโป้ง (normalize ด้วยขนาดมือ) ต่ำกว่านี้ = ใกล้กัน พร้อมดีด (armed)
const FLICK_FAR_ON = 0.75;         // ระยะสูงกว่านี้ = ห่างจากนิ้วโป้งชัดเจนแล้ว (fire)
const FLICK_MIN_FRAMES_ARMED = 2;  // ต้องใกล้กันต่อเนื่องกี่เฟรมก่อนนับว่า armed จริง (กัน false arm เฟรมเดียว)
const FLICK_MAX_ARM_AGE_MS = 1500; // armed ไว้นานเกินนี้โดยไม่แยกออก = ยกเลิก armed (กันค้างถาวร)
...
export async function createHandPinchInput(fxCanvas, handlers, onCameraLost) {
  ...
  // ระยะปลายนิ้วชี้ (8) ถึงปลายนิ้วโป้ง (4) normalize ด้วยขนาดมือ — สูตรเดียวกับ
  // rawDist/normDist ใน getPinchState ด้านล่าง (ระยะระหว่าง 2 จุดบนมือเดียวกัน
  // ไม่ขึ้นกับการหมุนข้อมือทั้งมือ ต่างจากการวัดนิ้วชี้เทียบกับโคนนิ้วชี้ตัวเองที่
  // เคยใช้มาก่อน ซึ่งไวต่อมุมกล้อง/การหมุนข้อมือเกินไป — ดูประวัติด้านบน)
  function thumbIndexDist(lm) {
  ...
  function updateFlickState(dist, x, y, tx, ty) {
    ...
    if (dist < FLICK_NEAR_ON) {
      // นิ้วชี้-นิ้วโป้งอยู่ใกล้กัน — ต้องใกล้ต่อเนื่องกี่เฟรมก่อนนับว่า armed จริง
      // (กัน false arm เฟรมเดียว)
    ...
    // ยิงดีดเฉพาะตอนเคยอยู่ใกล้กัน (armed) มาก่อนจริงๆ แล้วแยกห่างออกพอภายในเวลา
    // ที่กำหนดเท่านั้น — วัดจากระยะนิ้วชี้-นิ้วโป้งโดยตรง (ไม่ใช่นิ้วชี้เทียบกับตัวเอง
    // อีกต่อไป) ทำให้มือที่กำอยู่เฉยๆ แล้วหมุนข้อมือไม่ทำให้ค่านี้เปลี่ยนแปลงมาก
    // พอจะยิงได้ ต้องมีการแยกสองนิ้วออกจากกันจริงเท่านั้น (ดูประวัติการปรับจูนด้านบน)
    const armedAndFar = flickArmed && dist > FLICK_FAR_ON && (now - flickArmedAt) < FLICK_MAX_ARM_AGE_MS;
    if (armedAndFar && (now - lastFlickAt) > FLICK_COOLDOWN_MS) {
      lastFlickAt = now;
      handlers.onPick && handlers.onPick(sx, sy, GRAB_SLOP, { x: stx, y: sty });
      flickArmed = false;
      flickArmedFrames = 0;
    } else if (flickArmed && (now - flickArmedAt) > FLICK_MAX_ARM_AGE_MS) {
      // armed ค้างนานเกินไปโดยไม่แยกออกจริง (เช่นเด็กเผลอถือมือนิ่งๆ) — ยกเลิก
  ...
  function detectLoop(gen) {
    ...
    if (!paused && !videoEl.paused && videoEl.currentTime !== lastVideoTime) {
      ...
      if (hand) {
        ...
        if (mode === 'flick') {
          // โหมดเกมไพ่: ดีดนิ้วชี้แทนจีบสองนิ้ว
          const dist = thumbIndexDist(hand.lm);
          ...
          updateFlickState(dist, x, y, tx, ty);
