...
//
// ลำดับมาตรา (ง่าย → ยาก) อิงหลักการสอนภาษาไทย ป.1:
//   1) สระเดี่ยว คู่สั้น-ยาว (อะ-อา, อิ-อี, อุ-อู, แอะ-แอ, เอาะ-ออ)
//   2) สระเดี่ยว (ต่อ) — คู่ยาว-สั้น เอ-เอะ, อือ-อึ + สระเดี่ยว โอ, เออ
//   3) สระประสม — ผสม 2 เสียง เลื่อนลิ้น (เอีย, เอือ, อัว)
//   4) สระเกิน — มีเสียงตัวสะกดแฝงในตัว ปูทางสู่มาตราตัวสะกดจริง (อำ, เอา, ไอ)
//   5) มาตราตัวสะกดจริง — ตรงตัวก่อน (ง น ม ย ว) แล้วไม่ตรงตัว (บ ก ด)
//      แม่ กด ไว้ท้ายสุดเพราะมีพยัญชนะแทนเสียงมากที่สุด (ยากสุดในทางปฏิบัติ)
//
// หมายเหตุ TWO_PART ที่มีตัวสะกด (สระ เอ / อึ / เออ): สระพวกนี้แทบไม่มีคำพยางค์เปิด
// ที่ STT (th-TH) รู้จัก จึงใช้คำมีตัวสะกดเป็น display โดยผูก lead ↔ คำจริง 1:1 —
// ทุกฟองที่ลากลงหม้อได้จึงเป็นคำจริงเสมอ (ไม่ขัดกฎ "ทุกคู่เป็นคำจริง" ของ TWO_PART)
// ช่อง pill ย่อจะโชว์แค่ "lead + สระ = คำ" (ตัวสะกดไปโผล่ที่ spell/ทูลทิปผู้ปกครองแทน)
...
export const MATRA = [
  ...
  // ─── กลุ่ม 2: สระเดี่ยว (ต่อ) — เติมคู่ยาว-สั้น เอ/เอะ, อือ/อึ + โอ, เออ ─────
  {
    id: 'sara_e',
    name: 'สระ เอ',
    mode: 'TWO_PART',
    sessionSize: 6,
    sara: 'เ',
    // สระ เอ พยางค์เปิด (เ_) แทบไม่มีคำเดี่ยวที่ STT รู้จัก → ใช้คำมีตัวสะกดเป็น display
    // lead ผูกกับคำจริง 1 คำต่อ 1 ฟอง (ดูหมายเหตุหัวไฟล์)
    bubbles: ['ล','พ','ส','ก','ม','ว'],
    words: [
      // คำมีรูปวรรณยุกต์: สะกดคำไร้วรรณยุกต์ก่อน → เรียกชื่อวรรณยุกต์ → คำเต็ม (วิธีแจกลูก)
      { display:'เล่น', lead:'ล', sara:'เ', spell:['ลอ','เอ','นอ','เลน','ไม้เอก','เล่น'] },   // เล่นเกม
      { display:'เพลง', lead:'พ', sara:'เ', spell:['พอ','เอ','งอ','เพลง'] },   // ร้องเพลง
      { display:'เส้น', lead:'ส', sara:'เ', spell:['สอ','เอ','นอ','เสน','ไม้โท','เส้น'] },   // เส้นก๋วยเตี๋ยว
      { display:'เก่ง', lead:'ก', sara:'เ', spell:['กอ','เอ','งอ','เกง','ไม้เอก','เก่ง'] },   // เก่งมาก
      { display:'เม่น', lead:'ม', sara:'เ', spell:['มอ','เอ','นอ','เมน','ไม้เอก','เม่น'] },   // ตัวเม่น
      { display:'เว้น', lead:'ว', sara:'เ', spell:['วอ','เอ','นอ','เวน','ไม้โท','เว้น'] },   // เว้นวรรค
    ],
  },
  {
    id: 'sara_e_short',
    ...
    words: [
      ...
      // say: TTS อ่าน "เปะ" เพี้ยนเป็น "ปะ" — ใช้ "เป๊ะ" (คำในพจนานุกรม สระถูก) จนกว่าจะมีไฟล์เสียงอัด
      { display:'เปะ', lead:'ป', sara:'เะ', spell:['ปอ','เอะ','เปะ'], say:'เป๊ะ' },   // พอเปะ
    ],
  },
  {
    id: 'sara_ue',
  ...
  },
  {
    ...
    sessionSize: 6,
    sara: 'ึ',
    // สระ อึ พยางค์เปิดแทบไม่มีคำจริง (คึ นึ ตึ ไม่ใช่คำ) → ใช้คำมีตัวสะกดเป็น display
    // lead ผูกกับคำจริง 1 คำต่อ 1 ฟอง (ดูหมายเหตุหัวไฟล์)
    bubbles: ['ต','ล','น','ผ','ฝ','บ'],
    words: [
      { display:'ตึก', lead:'ต', sara:'ึ', spell:['ตอ','อึ','กอ','ตึก'] },   // ตึกสูง
      { display:'ลึก', lead:'ล', sara:'ึ', spell:['ลอ','อึ','กอ','ลึก'] },   // น้ำลึก
      { display:'นึก', lead:'น', sara:'ึ', spell:['นอ','อึ','กอ','นึก'] },   // นึกออก
      { display:'ผึ้ง', lead:'ผ', sara:'ึ', spell:['ผอ','อึ','งอ','ผึง','ไม้โท','ผึ้ง'] },   // ผึ้งน้อย
      { display:'ฝึก', lead:'ฝ', sara:'ึ', spell:['ฝอ','อึ','กอ','ฝึก'] },   // ฝึกเขียน
      { display:'บึง', lead:'บ', sara:'ึ', spell:['บอ','อึ','งอ','บึง'] },   // หนองบึง
    ],
  },
  {
    id: 'sara_oo',
  ...
  },
  {
    ...
    sessionSize: 6,
    sara: 'เ',
    // สระ เออ: คำพยางค์เปิด (เธอ เจอ) มีน้อย จึงผสมกับคำมีตัวสะกด (เดิน เงิน เกิน เนิน)
    // ตัวสะกดทั้งชุดเป็น น เพื่อให้ spell อ่านง่ายและไม่ต้องเดารูปสะกดยาก ๆ
    bubbles: ['ธ','จ','ด','ง','ก','น'],
    words: [
      { display:'เธอ', lead:'ธ', sara:'เ', spell:['ธอ','เออ','เธอ'] },          // เธอ (เพื่อน)
      { display:'เจอ', lead:'จ', sara:'เ', spell:['จอ','เออ','เจอ'] },          // เจอกัน
      { display:'เดิน', lead:'ด', sara:'เ', spell:['ดอ','เออ','นอ','เดิน'] },   // เดินเล่น
      { display:'เงิน', lead:'ง', sara:'เ', spell:['งอ','เออ','นอ','เงิน'] },   // เงินทอง
      { display:'เกิน', lead:'ก', sara:'เ', spell:['กอ','เออ','นอ','เกิน'] },   // เกินไป
      { display:'เนิน', lead:'น', sara:'เ', spell:['นอ','เออ','นอ','เนิน'] },   // เนินเขา
    ],
  },
  // ─── กลุ่ม 3: สระประสม (ผสม 2 เสียง เลื่อนลิ้น) — ยากขึ้น ────────────────────
  {
    id: 'sara_ia',
    name: 'สระ เอีย',
    mode: 'TWO_PART',
  ...
  // ─── กลุ่ม 4: สระเกิน — มีเสียงตัวสะกดแฝงในตัว ปูทางสู่มาตราตัวสะกดจริง ──────
  {
    id: 'sara_am',
    name: 'สระ อำ',
    mode: 'TWO_PART',
  ...
  },
  {
    ...
    sessionSize: 6,
    sara: 'ไ',
    // ไอ ไม้มลาย เท่านั้น (ใอ ไม้ม้วน เป็นเรื่องการสะกดยกเว้น ไม่ใช่เรื่องเสียง)
    bubbles: ['ก','ข','ฟ','ป','ม','ว'],
    words: [
      { display:'ไก่', lead:'ก', sara:'ไ', spell:['กอ','ไอ','ไก','ไม้เอก','ไก่'] },   // ไก่
      { display:'ไข่', lead:'ข', sara:'ไ', spell:['ขอ','ไอ','ไข','ไม้เอก','ไข่'] },   // ไข่
      { display:'ไฟ', lead:'ฟ', sara:'ไ', spell:['ฟอ','ไอ','ไฟ'] },    // ไฟ
      { display:'ไป', lead:'ป', sara:'ไ', spell:['ปอ','ไอ','ไป'] },    // ไป
      { display:'ไม้', lead:'ม', sara:'ไ', spell:['มอ','ไอ','ไม','ไม้โท','ไม้'] },   // ไม้
      { display:'ไว', lead:'ว', sara:'ไ', spell:['วอ','ไอ','ไว'] },    // เร็วไว
    ],
  },
  // ─── มาตราตัวสะกดจริง — ตรงตัวก่อน (ง น ม ย ว) แล้วไม่ตรงตัว (บ ก ด) ────────
  {
    id: 'kong',
    name: 'แม่ กง',
    mode: 'FILL_FINAL',
  ...
  },
  {
    ...
    words: [
      ...
      { display:'ยิ้ม', lead:'ยิ', final:'ม', spell:['ยอ','อิ','มอ','ยิม','ไม้โท','ยิ้ม'], distractors:['น','ว','ก'] },
    ],
  },
  {
    id: 'koei',
