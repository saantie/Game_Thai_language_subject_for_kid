...
const VOICE = {
  ...
  mic_retry: ['ลองอ่านอีกครั้ง', 'ลองใหม่อีกครั้ง'], // ไมค์ตัดจบโดยไม่ทันได้ยิน (เด็กพูดไม่ทัน)
  ...
  sfx(name) {
    ...
    switch (name) {
      ...
      case 'ting':
        this._ting(t);
        break;
      // มินิเกมไพ่: ไพ่ไหลเข้าที่ตอนเริ่มมาตรา (renderBoard) — ติ๊กสั้นๆ ต่อใบ
      // สุ่ม pitch เล็กน้อยกันฟังซ้ำจำเจตอนไพ่เยอะ (เล่นรัวถี่ตาม stagger เดิม)
      case 'card_deal':
        this._blip(480 + Math.random() * 60, 0.05, 'triangle', t);
        break;
      // มินิเกมไพ่: ไพ่ลอยไปถึงถาดแล้ว (flyToTray หลัง FLY_MS) — โน้ตคู่สั้นๆ
      // ให้ความรู้สึก "วางลง" ต่างจาก 'pick' ตอนเริ่มลอย
      case 'card_place':
      ...
      case 'tile_blocked':
        ...
        break;
      // แผนที่มนตรา: แม่มดน้อยเหวี่ยงไม้โดนลูกสมุน — "ฟวับ-ปั้ก" การ์ตูน
      case 'swing':
        this._blip(240, 0.05, 'sawtooth', t);
        this._blip(110, 0.11, 'triangle', t + 0.03);
        break;
      // แผนที่มนตรา: ลูกสมุนกัดแม่มดน้อย — "ง่ับ" ต่ำ ๆ 2 จังหวะ
      case 'bite':
      ...
      case 'minion_cry':
        ...
        break;
      // แผนที่มนตรา: แม่มดน้อยโดนกัด — เสียงร้อง "อ๊ะ!" เสียงสูงใส ตกลงเร็ว
      case 'hero_cry':
