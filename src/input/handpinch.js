...
// ดีดนิ้วชี้ (โหมด 'flick' — เฉพาะเกมไพ่จับคู่): งอนิ้วชี้ค้างไว้ก่อน (armed) แล้ว
// ยืดออก (fire) ต่างจากท่าจีบสองนิ้วที่ใช้ในเกมหยิบฟอง (โหมด 'pinch' เดิม)
//
// ประวัติการปรับจูน (ทั้งคู่มาจากทดสอบเครื่องจริง):
//   รอบ 1 (CURL 0.55/EXTEND 0.85/1 เฟรม/900ms) → "ดีดแล้วไม่ค่อยตอบสนอง" (ตึงไป)
//   รอบ 2 (CURL 0.62/EXTEND 0.72/1 เฟรม/1500ms + ทางลัดความเร็ว) → ตอบสนองดีมาก
//     แต่ "บางครั้งไพ่ลอยไปโดยไม่ตั้งใจ" (หลวมไป — โดยเฉพาะทางลัดความเร็วที่ไม่ต้อง
//     งอนิ้วเลยก็ยิงได้ แค่ขยับมือเร็วพอ ก็เข้าเงื่อนไข false positive ง่ายสุด)
// รอบนี้ (รอบ 3): ตัดทางลัดความเร็วออกทั้งหมด (สาเหตุ false positive ที่น่าจะเป็น
// ไปได้มากที่สุด เพราะไม่ต้องงอนิ้วเลยก็ยิงได้) กลับไปพึ่งจังหวะงอ→ยืดอย่างเดียว
// แล้วปรับ threshold มาเป็นค่ากลางระหว่าง 2 รอบก่อน + เพิ่มจำนวนเฟรมยืนยัน armed
// กลับเป็น 2 (กันงอนิ้วแวบเดียวจากมือสั่น/เคลื่อนที่ปกตินับเป็น armed ผิดๆ)
// [ค่าทั้งหมดยังเป็นค่าประมาณ ต้องทดสอบบนอุปกรณ์จริงต่อ — ถ้ายังเจอ false positive
// อีกให้ลองขยับ FLICK_CURL_ON ลง (เข้มขึ้น) หรือเพิ่ม FLICK_MIN_FRAMES_ARMED อีก]
const FLICK_CURL_ON = 0.58;        // extension (ดูฟังก์ชัน indexExtension) ต่ำกว่านี้ = นิ้วงอ พร้อมดีด
const FLICK_EXTEND_ON = 0.80;      // extension สูงกว่านี้ = นิ้วยืดออกแล้ว (fire)
const FLICK_MIN_FRAMES_ARMED = 2;  // ต้องงอต่อเนื่องกี่เฟรมก่อนนับว่า armed จริง (กัน false arm เฟรมเดียว)
...
export async function createHandPinchInput(fxCanvas, handlers, onCameraLost) {
  ...
  function updateFlickState(ext, x, y, tx, ty) {
    ...
    // ยิงดีดเฉพาะตอนเคยงอ (armed) มาก่อนจริงๆ แล้วยืดออกพอภายในเวลาที่กำหนดเท่านั้น
    // (ตัดทางลัดตรวจจับความเร็วเคลื่อนที่ออกแล้ว — เคยเพิ่มไว้เป็นตัวสำรอง แต่เป็น
    // สาเหตุหลักที่ทำให้ไพ่ลอยไปเองตอนแค่ขยับมือโดยไม่ได้ตั้งใจดีด เพราะไม่ต้องงอ
    // นิ้วเลยก็ยิงได้ — ดูประวัติการปรับจูนด้านบน)
    const armedAndExtended = flickArmed && ext > FLICK_EXTEND_ON && (now - flickArmedAt) < FLICK_MAX_ARM_AGE_MS;
    if (armedAndExtended && (now - lastFlickAt) > FLICK_COOLDOWN_MS) {
      lastFlickAt = now;
      handlers.onPick && handlers.onPick(sx, sy, GRAB_SLOP, { x: stx, y: sty });
      flickArmed = false;
      flickArmedFrames = 0;
    ...
    if (!paused && !videoEl.paused && videoEl.currentTime !== lastVideoTime) {
      ...
      if (hand) {
        ...
        if (mode === 'flick') {
          ...
          updateFlickState(ext, x, y, tx, ty);
