---
description: step-by-step guide implement handpinch.js ด้วย MediaPipe Hand Landmarker — camera permission, sticky hand lock (กันหลายมือ/หลายเด็ก), pinch hysteresis + smoothing สำหรับมือเด็ก, hand-size normalization, magnet grab, hybrid touch+AR, กู้คืนเมื่อกล้องถูกตัด, ทำงานร่วม pointer mode
---
...
- จอดับ/สลับแอปแล้วกล้องถูก Android ตัด → กู้คืน/fallback (Step 1)
- offline → import CDN ล้มเหลวแบบนุ่มนวล ไม่ crash (Step 2)
- ช่วงฟังเสียงเด็กอ่าน (LISTENING) → pause inference คืน CPU ให้ STT (Step 4 + 6)
...
async function initCamera(videoEl, onCameraLost) {
  try {
    ...
    // ★ Android ตัด camera track เงียบ ๆ เมื่อจอดับ/สลับแอป/มีสายเข้า —
    //   ถ้าไม่ฟัง event นี้ เกมจะค้างสภาพ "AR เปิดอยู่แต่มือใช้ไม่ได้"
...
เมื่อ `onCameraLost` ถูกเรียก main.js ต้อง `destroy()` ตัว AR input + เล่นเสียงแม่มด
แจ้งสั้น ๆ (เช่น "ใช้นิ้วจิ้มจอแทนได้เลยจ้ะ") — touch ยังทำงานอยู่แล้วเพราะเป็น hybrid (Step 6)

คง 640×480 ไว้ — พอสำหรับ hand landmark และเบาพอสำหรับ Android GPU กลาง-ล่าง
อย่าขอ resolution สูงกว่านี้ inference จะช้าลงโดยความแม่นไม่เพิ่ม
...
// ★ pause inference ช่วงฟังเสียงเด็กอ่าน — ผูกจุดเดียวกับ duck/unduck (สเปก §4.3)
function startListening() { duck(); arInput && arInput.pause(); recog.start(); }
recog.onend = () => { unduck(); arInput && arInput.resume(); };
```
...
- [ ] จอดับ/สลับแอปแล้วกลับมา → เกมยังเล่นได้ (touch ทำงาน, มีเสียงแจ้งถ้า AR หลุด)
- [ ] เปิดเกมแบบ offline → เกมเล่นได้ปกติด้วย touch, ไม่ crash, ไม่ค้างจอโหลด AR
- [ ] ระหว่างรอบฟังเสียง (ไมค์ทำงาน) → inference หยุดจริง (CPU ลด, ไม่มี onPick หลุดมา)
